...
dfts_demand_nsw %>% 
  mutate(diff_TOTALDEMAND = difference(TOTALDEMAND, 48)) %>% 
  ACF(diff_TOTALDEMAND, lag_max = 336) %>% 
  autoplot()

dfts_demand_nsw %>% 
  mutate(diff_TOTALDEMAND = difference(TOTALDEMAND, 48)) %>% 
  mutate(diff2_TOTALDEMAND = difference(diff_TOTALDEMAND, 48)) %>% 
  ACF(diff2_TOTALDEMAND, lag_max = 336) %>% 
  autoplot()
```
...
train_set <- dfts_nsw[1:(n-48*7), ]
test_set <- dfts_nsw[(n-48*7-1):n, ]
```
...
```{R}
  combined_data <- df %>%
    left_join(forecasts_fr, by = "DATETIME") %>% 
