...
ACF(dfts_demand_nsw) %>% autoplot() 
PACF(dfts_demand_nsw) %>% autoplot() 
...
initial_train_size <- nrow(df_nsw) - 48
...
# Initialize vectors to store forecasts and actual values for the 10 validation steps
forecasts <- numeric(10)
actuals <- numeric(10)
forecasts <- numeric(5)
actuals <- numeric(5)

for (i in 1:5) {  # Walk foward 10-times
  # Update training set to include one more observation each time
  train_set <- df_nsw[1:(initial_train_size + i - 1), ]
  
  # Fit the SARIMA model with the current training set
  fit <- auto.arima(train_set$TOTALDEMAND, xreg = train_set$TEMPERATURE, seasonal = TRUE, stepwise = FALSE, approximation = FALSE)
  
  # Forecast the next step with the corresponding temperature as the external regressor
  next_temp <- df_nsw$TEMPERATURE[initial_train_size + i]  # Next temperature value
  forecast_result <- forecast(fit, xreg = next_temp, h = 1)
  
  # Store the forecast and the actual demand for later comparison
  forecasts[i] <- forecast_result$mean
  actuals[i] <- df_nsw$TOTALDEMAND[initial_train_size + i]
}

# forecasts and actuals now contain the 48-step (24-hour) forecast and actual values
...
# Calculating MAPE, ensuring to avoid division by zero
...
# Assuming 'forecasts' and 'actuals' vectors are available from your earlier code

# Convert forecasts and actuals into a data frame for plotting
results_df <- data.frame(Time = 1:10, Forecast = forecasts, Actual = actuals)

# Melting the data frame for easier plotting with ggplot

melted_results_df <- melt(results_df, id.vars = "Time", variable.name = "Type", value.name = "Demand")

# Plotting with ggplot
ggplot(data = melted_results_df, aes(x = Time, y = Demand, color = Type)) +
  geom_line() +
  geom_point() +
  theme_minimal() +
  labs(title = "Forecast vs Actual Demand",
       x = "Time Step (hours)",
       y = "Demand",
       color = "Legend") +
  scale_color_manual(values = c("Forecast" = "blue", "Actual" = "red"))

# This plot will show the forecasted demand in blue and the actual demand in red for comparison.
...
```{r}
model <- auto.arima(train_set$TOTALDEMAND, xreg=train_set$TEMPERATURE, seasonal = TRUE, stepwise = FALSE, approximation = FALSE)
...
dfts_nsw <- df_nsw %>% 
  ...
  as_tsibble(index = DATETIME, key=key)
...
p,d,q = 3,1,2 from auto.arima() function

P: spike lag 1 in PACF
D: Strong seasonal effect
Q: ACF
```{r}
sarima_model_312_111_48 <- dfts_nsw %>%
  model(SARIMAX = ARIMA(TOTALDEMAND ~ TEMPERATURE + pdq(3, 1, 2) + PDQ(1, 1, 1, 48)))
...
sarima_model_312_111_48<- df_nsw %>%
  model(SARIMAX(TOTALDEMAND ~ TEMPERATURE + PDQ(3, 1, 2) + SPQ(1, 1, 1, 48)))
